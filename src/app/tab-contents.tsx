...
const TabLayout = ({ hasStamps, stamps, score }: { hasStamps: boolean, stamps: any[], score: number }) => {
    return (
        <Tabs>
            <TabList>
                <Tab>Home</Tab>
...
const ShowStamps = ({ stamps }: { stamps: any[] }) => {
    if (stamps.length > 0) {
        return (
            <>
                <br />
...
const CheckStamps = ({ hasStamps }: { hasStamps: boolean }) => {
    if (hasStamps) {
        return (
